...
class GenericLLDP(app_manager.RyuApp):
    ...
    def port_desc_stats_reply_handler(self, ev):
        ...
        for stat in ev.msg.body:
            if stat.port_no < ofproto.OFPP_MAX:
                match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_LLDP, in_port = ofproto.OFPP_CONTROLLER)
                actions.append(parser.OFPActionSetField(eth_src=stat.hw_addr))
                actions.append(parser.OFPActionOutput(stat.port_no))
    ...
    def send_lldp_packet(self, datapath):
        ...
        pkt.add_protocol(ethernet.ethernet(ethertype=ether_types.ETH_TYPE_LLDP, src="00:00:00:00:00:00", dst=lldp.LLDP_MAC_NEAREST_BRIDGE))
        ...
        tlv_port_id = lldp.PortID(subtype=lldp.PortID.SUB_LOCALLY_ASSIGNED, port_id=self.format_port("0"))
